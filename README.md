...
- 生成对抗网络( Generative Adversarial Networks，GANs )对复杂分布的采样过程进行建模，以对抗的方式进行学习
- 另一类生成式模型，称为"似然法"，旨在学习一个模型，该模型为观察到的数据样本分配较高的可能性。这包括自回归模型，正规化流和变分自编码器( VAEs )
- 另一种类似的方法是基于能量的建模，将分布学习为任意灵活的能量函数，然后对其进行归一化
